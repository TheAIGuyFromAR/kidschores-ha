...
async def async_setup_entry(
    ...
    for kid_id, kid_info in coordinator.kids_data.items():
        ...
        # Pending Chores Count per Kid
        entities.append(KidChoresPendingCountSensor(coordinator, entry, kid_id, kid_name))

        # Overdue Chores Count per Kid
        entities.append(KidChoresOverdueCountSensor(coordinator, entry, kid_id, kid_name))

        # Weekly Allowance per Kid
        entities.append(KidWeeklyAllowanceSensor(coordinator, entry, kid_id, kid_name))
...
# ------------------------------------------------------------------------------------------
class KidChoresPendingCountSensor(CoordinatorEntity, SensorEntity):
    """Sensor for counting pending chores for a kid."""

    _attr_has_entity_name = True
    _attr_translation_key = "chores_pending_count_sensor"
    ...
    def __init__(self, coordinator, entry, kid_id, kid_name):
        ...
        self._kid_id = kid_id
        self._kid_name = kid_name
        self._attr_unique_id = f"{entry.entry_id}_{kid_id}_chores_pending_count"
        self._attr_translation_placeholders = {
            "kid_name": kid_name,
        }
        self.entity_id = f"sensor.kc_{kid_name}_chores_pending_count"

    @property
    def native_value(self):
        """Return the count of pending chores for this kid."""
        kid_info = self.coordinator.kids_data.get(self._kid_id, {})
        count = 0

        # Count chores assigned to this kid that are pending
        for chore_id, chore_info in self.coordinator.chores_data.items():
            if self._kid_id not in chore_info.get("assigned_kids", []):
                continue

            # Check if chore is pending (not claimed, not approved, not overdue)
            if (chore_id not in kid_info.get("claimed_chores", []) and
                chore_id not in kid_info.get("approved_chores", []) and
                chore_id not in kid_info.get("overdue_chores", [])):
                count += 1
    ...
    @property
    def icon(self):
        """Return icon for pending chores."""
        return "mdi:clipboard-text-clock-outline"


class KidChoresOverdueCountSensor(CoordinatorEntity, SensorEntity):
    """Sensor for counting overdue chores for a kid."""

    _attr_has_entity_name = True
    _attr_translation_key = "chores_overdue_count_sensor"
    ...
    def __init__(self, coordinator, entry, kid_id, kid_name):
        ...
        self._kid_id = kid_id
        self._kid_name = kid_name
        self._attr_unique_id = f"{entry.entry_id}_{kid_id}_chores_overdue_count"
        self._attr_translation_placeholders = {
            "kid_name": kid_name,
        }
        self.entity_id = f"sensor.kc_{kid_name}_chores_overdue_count"

    @property
    def native_value(self):
        """Return the count of overdue chores for this kid."""
        kid_info = self.coordinator.kids_data.get(self._kid_id, {})
        return len(kid_info.get("overdue_chores", []))

    @property
    def icon(self):
        """Return icon for overdue chores."""
        return "mdi:clipboard-alert-outline"


class KidWeeklyAllowanceSensor(CoordinatorEntity, SensorEntity):
    """Sensor for calculating weekly allowance based on chore completion."""

    _attr_has_entity_name = True
    _attr_translation_key = "weekly_allowance_sensor"
    ...
    def __init__(self, coordinator, entry, kid_id, kid_name):
        ...
        self._kid_id = kid_id
        self._kid_name = kid_name
        self._attr_unique_id = f"{entry.entry_id}_{kid_id}_weekly_allowance"
        self._attr_translation_placeholders = {
            "kid_name": kid_name,
        }
        self.entity_id = f"sensor.kc_{kid_name}_weekly_allowance"
        self._attr_native_unit_of_measurement = "$"

    @property
    def native_value(self):
        """Return the weekly allowance based on completion percentage."""
        kid_info = self.coordinator.kids_data.get(self._kid_id, {})

        # Get weekly points earned
        weekly_points = kid_info.get("points_earned_weekly", 0)

        # Base allowance could be configured, defaulting to weekly points as dollar value
        # This can be adjusted based on your specific allowance calculation
        return round(weekly_points * 0.10, 2)  # 10 cents per point

    @property
    def icon(self):
        """Return icon for allowance."""
        return "mdi:cash"

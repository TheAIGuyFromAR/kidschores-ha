...
class KidsChoresDataCoordinator(DataUpdateCoordinator):
    ...
    async def async_config_entry_first_refresh(self):
        ...
        # Initialize missing due dates for recurring chores
        self._initialize_missing_due_dates()
    ...
    def _initialize_missing_due_dates(self):
        """Initialize due dates for recurring chores that don't have one."""
        now_local = dt_util.utcnow().astimezone(
            dt_util.get_time_zone(self.hass.config.time_zone)
        )

        for chore_id, chore_info in self._data.get(DATA_CHORES, {}).items():
            freq = chore_info.get("recurring_frequency", FREQUENCY_NONE)

            # Skip if not recurring or already has a due date
            if freq == FREQUENCY_NONE or chore_info.get("due_date"):
                continue

            # Calculate due date based on applicable_days
            applicable_days = chore_info.get("applicable_days", [])
            due_date = self._calculate_next_due_date(now_local, freq, applicable_days)
            chore_info["due_date"] = due_date.isoformat()

            LOGGER.info(
                "Initialized missing due_date for chore '%s' (freq=%s) to %s",
                chore_info.get("name", chore_id),
                freq,
                chore_info["due_date"],
            )

    def _calculate_next_due_date(self, now_local, frequency, applicable_days):
        """Calculate the next due date based on frequency and applicable days."""
        # Default to 23:59 today
        due_dt = now_local.replace(hour=23, minute=59, second=0, microsecond=0)

        # If due time has passed, start from tomorrow
        if due_dt <= now_local:
            due_dt += timedelta(days=1)

        # If there are applicable days, find the next matching day
        if applicable_days:
            day_map = {'mon': 0, 'tue': 1, 'wed': 2, 'thu': 3, 'fri': 4, 'sat': 5, 'sun': 6}
            target_days = [day_map.get(d.lower(), -1) for d in applicable_days if d.lower() in day_map]

            if target_days:
                current_weekday = due_dt.weekday()
                days_ahead = 0
                while days_ahead <= 7:
                    if (current_weekday + days_ahead) % 7 in target_days:
                        break
                    days_ahead += 1
    ...
    def _initialize_data_from_config(self):
        ...
        # DEBUG: Log how many chores we're loading
        LOGGER.warning(f"🔍 DEBUG: Loading {len(options.get(CONF_CHORES, {}))} chores from config_entry options")
        LOGGER.warning(f"🔍 DEBUG: Options keys: {list(options.keys())}")
        ...
        # DEBUG: Check if chores made it into self._data
        LOGGER.warning(f"🔍 After initialization, self._data[DATA_CHORES] has {len(self._data.get(DATA_CHORES, {}))} chores")
        LOGGER.warning(f"🔍 chores_data property returns: {len(self.chores_data)} chores")
    ...
    def _initialize_chores(self, chores_dict: dict[str, Any]):
        LOGGER.warning(f"🔍 _initialize_chores called with {len(chores_dict)} chores")
        for chore_id, chore_data in list(chores_dict.items())[:3]:
            LOGGER.warning(f"🔍   Sample chore: {chore_data.get('name', 'NO NAME')}")
        self._sync_entities(
    ...
    def _create_chore(self, chore_id: str, chore_data: dict[str, Any]):
        assigned_kids_ids = []
        for kid_name_or_id in chore_data.get("assigned_kids", []):
            # Support both kid names (legacy) and kid UUIDs (new format)
            if kid_name_or_id in self.kids_data:
                # It's already a UUID
                assigned_kids_ids.append(kid_name_or_id)
            else:
                # Try looking it up by name
                kid_id = self._get_kid_id_by_name(kid_name_or_id)
                if kid_id:
                    assigned_kids_ids.append(kid_id)
                else:
                    LOGGER.warning(
                        "Chore '%s': Kid '%s' not found. Skipping assignment",

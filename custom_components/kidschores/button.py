...
class ClaimChoreButton(CoordinatorEntity, ButtonEntity):
    ...
    def _get_chore_state(self):
        """Determine the current state of this chore."""
        ...
        # Check awaiting approval first (highest priority display state)
        is_awaiting_approval = self._chore_id in kid_info.get("claimed_chores", [])
        if is_awaiting_approval:
            return "awaiting_approval"

        # Check if overdue
        is_overdue = self._chore_id in kid_info.get("overdue_chores", [])
        if is_overdue:
            return "overdue"

        # Check due date for today/upcoming
        due_date_str = chore_info.get("due_date", DUE_DATE_NOT_SET)
        is_today = False
        is_upcoming = False
        if due_date_str and due_date_str != DUE_DATE_NOT_SET:
            try:
                due_date = datetime.fromisoformat(due_date_str.replace("Z", "+00:00"))
                today = datetime.now().date()
                is_today = due_date.date() == today
                is_upcoming = due_date.date() > today
            except (ValueError, TypeError):
                pass

        if is_today:
            # Check if it's a bonus chore
            is_bonus = any("bonus" in label.lower() for label in friendly_labels)
            return "bonus_today" if is_bonus else "today"

        if is_upcoming:
            is_bonus = any("bonus" in label.lower() for label in friendly_labels)
            return "bonus_upcoming" if is_bonus else "upcoming"

        # Default state - available bonus opportunity
        is_bonus = any("bonus" in label.lower() for label in friendly_labels)
        return "bonus_available" if is_bonus else "available"

    def _handle_coordinator_update(self) -> None:
        """Handle updated data from the coordinator."""
        # Update icon based on current state
        state = self._get_chore_state()
        if state == "awaiting_approval":
            self._attr_icon = "mdi:clock-outline"
        elif state == "overdue":
            self._attr_icon = "mdi:alert-circle"
        else:
            self._attr_icon = self._base_icon
    ...
    def extra_state_attributes(self):
        ...
        chore_state = self._get_chore_state()

        # Determine if chore is a bonus (has "Bonus" label)
        is_bonus = any(
            "bonus" in label.lower() for label in friendly_labels
        )

        # Get last claimed by info from pending approvals
        last_claimed_by = None
        pending_approvals = self.coordinator._data.get(DATA_PENDING_CHORE_APPROVALS, [])
        for approval in pending_approvals:
            if approval.get("kid_id") == self._kid_id and approval.get("chore_id") == self._chore_id:
                last_claimed_by = approval.get("claimed_by", self._kid_name)
                break

        # Extract individual states from chore_state for backward compatibility
        is_overdue = chore_state == "overdue"
        is_today = chore_state in ("today", "bonus_today")
        is_upcoming = chore_state in ("upcoming", "bonus_upcoming")
        is_awaiting_approval = chore_state == "awaiting_approval"

        attributes = {
            ATTR_LABELS: friendly_labels,
            "chore_name": chore_info.get("name", self._chore_name),
            "chore_state": chore_state,
            "overdue": is_overdue,
            "bonus": is_bonus,
            "today": is_today,
            "upcoming": is_upcoming,
            "awaiting_approval": is_awaiting_approval,
            "last_claimed_by": last_claimed_by,
        }
